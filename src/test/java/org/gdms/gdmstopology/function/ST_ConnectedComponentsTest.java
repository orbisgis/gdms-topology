...
/**
 * Tests the calculation of connected components on an <b>undirected</b> graph.
 *
 * @author Adam Gouge
 */
public class ST_ConnectedComponentsTest extends TopologySetupTest {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(ST_ConnectedComponentsTest.class);

    @Test
    public void test() throws Exception {
        DataSet[] tables = new DataSet[]{prepareEdges()};
        DataSet result = new ST_ConnectedComponents()
                .evaluate(dsf, tables, null,
    ...
    private DataSet prepareEdges() throws Exception {
        ...
        data.addValues(new Value[]{
            ValueFactory.createValue(
            wktReader.read("LINESTRING(2 1, 2 2)")),
            ValueFactory.createValue(2)});
        data.addValues(new Value[]{
            ValueFactory.createValue(
            wktReader.read("LINESTRING(2 2, 1 2)")),
        ...
        data.addValues(new Value[]{
            ValueFactory.createValue(
            wktReader.read("LINESTRING(3 2, 4 1)")),
            ValueFactory.createValue(5)});
        data.addValues(new Value[]{
            ValueFactory.createValue(
            wktReader.read("POINT(5 2)")),
